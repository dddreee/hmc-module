...
typedef struct tagMIMEBLOCK
{
	...
	/// Type name
	char		ctype[ MIME_STRSIZE ];

	/// Filename
	char		fname[ MIME_STRSIZE ];							   

	/// Boundry string
	char		boundry[ MIME_STRSIZE ];

	/// CID
	char		cid[ MIME_STRSIZE ];
...
/**
  ...
  \code
	...
	// Extract as html and plain text
	mime.ExtractHtml( "Test_001.htm", "Test_001.txt", "./cid", "./attachments" );
...
public:
	...
	// GetTextBlocks()
	//==============================================================
	/// Finds common e-mail components
	/**
		\param [in] pMime		-	MIME document
		\param [in] plaintext	-	Receives plain text part
		\param [in] htmltext	-	Receives html part
		\param [in] multipart	-	Receives multi-part
		
		Searches the MIME document for the specified common components.
	...
	*/
	...
	/// Finds common e-mail components
	/**
		\param [in] pMulti		-	Receives multi-part
		\param [in] plaintext	-	Receives plain text part
		\param [in] htmltext	-	Receives html part
		...
		\see 
	*/
	BOOL GetTextBlocks(CMime *pMulti, LPMIMEBLOCK *plaintext, LPMIMEBLOCK *htmltext);
...
public:
	...
	/// 
	/**
		\param [in] pHtmlFile			-	HTML file name that will
											be created containing any
											the first HTML content 
											part in the MIME document.
											If NULL, HTML data is not
											extracted.
		\param [in] pPlainText			-	If not NULL, the first plain
											text part will be extracted
											to the specified file.
		\param [in] pCidFolder			-	Folder in which to extract
											any embedded cid components.
											If this folder does not exist,
											it will be created.
		\param [in] pAttachmentsFolder	-	Folder in which to extract any
											attachments.  If this folder
		...
		\see 
	*/
	BOOL ExtractHtml( LPCTSTR pHtmlFile, LPCTSTR pPlainText, LPCTSTR pCidFolder, LPCTSTR pAttachmentsFolder );
